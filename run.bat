...
:end
echo.
echo OrbitoDB has stopped.
pause
endlocal
exit /b 0

REM --- free port 1420 if a previous Vite / OrbitoDB instance is still holding it ---
:freeport
set "PORT_BUSY="
for /f "tokens=5" %%p in ('netstat -ano ^| findstr /r /c:":1420 " ^| findstr "LISTENING"') do set "PORT_BUSY=1"
if not defined PORT_BUSY goto :eof
echo.
echo Port 1420 is already in use - another Vite / OrbitoDB instance is probably running.
